        ...
        <div class="container mt-4">
            <div class="border p-4">
                ...
                <div class="summary">
                    ...
                    <p><span>ID: {{ $article->id }}</span> / <time>{{ $article->post_date }}</time> / feeling:
                        {{ $article->feeling_before }} → {{ $article->feeling_after }}</p>
                </div>
            ...
            <div class="form-group text-right">
                <form action="{{ route('articles.destroy', ['article' => $article]) }}" method="post" class="ml-2">
                    <a class="btn btn-success btn-sm" href="{{ route('articles.edit', ['article' => $article]) }}"
                        role="button">Edit</a>
